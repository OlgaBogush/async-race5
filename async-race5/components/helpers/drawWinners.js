export default function drawWinners(array) {
  const tbody = document.querySelector("tbody")
  tbody.innerHTML = ""

  array.forEach(({ id, wins, time, name, color }) => {
    // rowCar
    const rowCar = document.createElement("tr")
    rowCar.classList.add("row-car")
    rowCar.dataset.winnerId = id

    const rowCarNumber = document.createElement("td")
    rowCarNumber.textContent = `id: ${id}`

      const svgCar = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
